...
//    }
    ...
    public void addFirst(T p){
        ...
        size += 1;
        if (size >= items.length) resize("larger");
    }

    public void addLast(T p){
        items[nextLast] = p;
        nextLast += 1;
        if (nextLast >= items.length) nextLast = nextLast - items.length;
        size += 1;
        if (size >= items.length) resize("larger");
    }
    ...
    public T removeFirst(){
        ...
        size -= 1;
        if((double) size / items.length < 0.25) resize("smaller");
        return t;
    }
